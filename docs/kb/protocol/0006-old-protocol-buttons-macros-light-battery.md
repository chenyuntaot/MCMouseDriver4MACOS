...
| 2 | 键盘键（含组合键） | `[修饰键掩码, HID Usage ID, 0x00]`，修饰掩码=标准 HID（bit0 LCtrl…bit3 LGUI），如 Ctrl+A=`0x010400`；完整键表 `Voe` 77119-77262 |
...
### 1.3 写单键 `0x12 0x52`（parser `fMt` 76836）
...
### 2.2 宏事件编码（逻辑值，4 字节定长，77776-77807）
